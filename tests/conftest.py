...
* ``d0_results``: real legacy models and outputs (CHROMBPNET_D0_RESULTS, e.g. /marimo/data/test_data_d0/results
  on the molab box); skipped when unset.
"""
import os
...
def golden_path(goldens_dir):
    """golden_path("pred_bias_065.npz") -> Path inside goldens_dir; skips the test if that file is missing."""
    def get(name):
...
@pytest.fixture(scope="session")
def d0_results():
    """Legacy d0 results tree with real bias / chrombpnet models (CHROMBPNET_D0_RESULTS)."""
    return _env_dir("CHROMBPNET_D0_RESULTS")
