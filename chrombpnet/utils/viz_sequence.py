# Sequence-logo helpers vendored from TF-MoDISco 0.5.16.0 (MIT License, Kundaje lab):
#   modisco/visualization/viz_sequence.py and modisco/util.py:compute_per_position_ic
# The legacy `modisco` 0.5.x package cannot be installed next to modisco>=2 (same PyPI name),
# so the few functions ChromBPNet uses are kept here with identical numerics and defaults.
# Additions: `background` may be any array-like, plot_weights_given_ax takes an optional ylim,
# and plot_weights returns the figure instead of calling plt.show().
import matplotlib
...
def plot_weights_given_ax(ax, array,
    ...
    if ylim is not None:
        # user-specified y-axis limits (not in modisco 0.5.16)
