...
- `--batch-seqs`: regions per device step (default: from the model's activation size and the free device memory, ~12 GB steps; e.g. 2 for chrombpnet_nobias 512x8, 12-18 for a 128x4 bias model); halved automatically when the GPU runs out of memory.
- `--precision` (default `auto`): full float32 on CPU, TF32 on GPU (as chrombpnet 1.x did). `highest` forces full float32 convolutions on the GPU too (exact to ~1e-7 against 1.x on CPU, but ~300x slower on an RTX PRO 6000 Blackwell); `default` always uses the backend default. TF32 scores agree with float32 ones to rel. L2 ~4e-4 and per-region Pearson > 0.99999.
